    ...
    (if (< (length (qtree-elements qtree)) (qtree-capacity qtree))
        ...
        (progn
          ...
          (dolist (sub-tree (qtree-sub-trees qtree))
            (qtree-insert sub-tree e))
          t))))

(defun qtree-subdivide (qtree)
  (when (qtree-sub-trees qtree)
    (error "QTREE sub-divided already!"))
  (setf (qtree-sub-trees qtree)
        (destructuring-bind ((x y) w h) (qtree-bbox qtree)
          (let ((w-new (/ w 2))
            ...
            (list (make-qtree (make-bbox x             y             w-new h-new) capacity)
                  (make-qtree (make-bbox (+ x (/ w 2)) y             w-new h-new) capacity)
                  (make-qtree (make-bbox x             (+ y (/ h 2)) w-new h-new) capacity)
                  (make-qtree (make-bbox (+ x (/ w 2)) (+ y (/ h 2)) w-new h-new) capacity))))))
